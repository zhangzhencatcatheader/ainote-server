...
项目支持企业/组织级别的多租户管理：
- **Company**: 企业/组织实体，每个企业有唯一的租户标识
- **AccountCompanyEntity**: 账户与企业的关联关系，支持多种角色
- **角色管理**: 支持 USER、ADMIN、COMPANYADMIN 等角色
- **成员管理**: 企业管理员可以管理企业成员和权限
...
- **手机短信验证码登录**: 基于阿里云短信服务的验证码登录/注册
- **图形验证码**: 使用 Hutool Captcha 生成图形验证码，防止机器人攻击
- **JWT 认证**: 基于令牌的无状态认证，支持令牌自动刷新
- **多租户选择**: 用户可以加入多个企业/租户，支持切换当前租户

### AI 集成能力

- **Spring AI 框架**: 集成 Spring AI，支持多种 AI 模型
- **Qwen (通义千问)**: 使用阿里云通义千问模型进行智能对话
- **OpenAI 集成**: 支持 OpenAI GPT 模型
- **智能模板生成**: AI 根据文档自动生成台账模板字段定义
- **文件理解**: 支持上传文件，AI 基于文件内容生成结构化数据
